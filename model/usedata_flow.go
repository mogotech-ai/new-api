...
func getAdminFlowQuotaData(startTime int64, endTime int64, username string) ([]*FlowQuotaData, error) {
	...
	query := flowQuotaBaseQuery(startTime, endTime).
		Select("user_id, username, use_group, " + adminQuotaModelExpr + " as model_name, channel_id, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used")
	if username != "" {
		query = query.Where("username = ?", username)
	}
	err := query.
		Group("user_id, username, use_group, " + adminQuotaModelExpr + ", channel_id").
		Order("quota DESC").
		Find(&rows).Error
	if err != nil {
		return nil, err
...
func getRootFlowQuotaData(startTime int64, endTime int64, username string) ([]*FlowQuotaData, error) {
	rows := make([]*FlowQuotaData, 0)
	query := flowQuotaBaseQuery(startTime, endTime).
		Select("user_id, username, node_name, token_id, use_group, " + adminQuotaModelExpr + " as model_name, channel_id, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used")
	if username != "" {
		query = query.Where("username = ?", username)
	}
	err := query.
		Group("user_id, username, node_name, token_id, use_group, " + adminQuotaModelExpr + ", channel_id").
		Order("quota DESC").
		Find(&rows).Error
	if err != nil {
		return nil, err
