...
func SetChannelBalance(c *gin.Context) {
	...
	if err := common.DecodeJson(c.Request.Body, &request); err != nil || request.Balance == nil {
		common.ApiErrorMsg(c, "balance is required")
		return
	}
	if *request.Balance < 0 || math.IsNaN(*request.Balance) || math.IsInf(*request.Balance, 0) {
		common.ApiErrorMsg(c, "balance must be finite and non-negative")
		return
	}
	...
	if err := channel.SetLocalBalance(*request.Balance); err != nil {
		common.ApiError(c, err)
		return
	}
